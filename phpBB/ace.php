...
*
* Adapted to add "Chat" type functions by Pre in 2012.
* We display the last X posts (either to all forums, a given
* forum, or a given thread) and a quick-reply box which
* (if it's all forums or a whole forum) has checkboxes to
* say which thread to reply to.
*
* We update that every few seconds, writing directly
* in to DOM if any new posts have shown up.
*
*
*/
...
// Extra stuff for ACE
$aceforum_id	= request_var('f', 0);
$acetopic_id	= request_var('t', 0);
$ace_topic_title = "ACE Page: ";
$ace_min_id=request_var('min',0);

//Couple of functions it'll want to use to grab extra data.
$topicTitleCache = array();
function getTopicTitle($x){
  global $db,$topicTitleCache;
  if(isset($topicTitleCache[$x])){
    return $topicTitleCache[$x];
  }
  $result = $db->sql_query("select topic_title as topic_title from ". TOPICS_TABLE ." where topic_id = $x");
  $topic_title = $db->sql_fetchfield('topic_title');
  $db->sql_freeresult($result);
  $topicTitleCache[$x]=$topic_title;
  return $topic_title;
}
$forumNameCache = array();
function getForumName($x){
  global $db,$forumNameCache;
  if(isset($forumNameCache[$x])){
    return $forumNameCache[$x];
  }
  $result = $db->sql_query("select forum_name as forum_name from ". FORUMS_TABLE ." where forum_id = $x");
  $forum_name = $db->sql_fetchfield('forum_name');
  $db->sql_freeresult($result);
  $forumNameCache[$x]=$forum_name;
  return $forum_name;
}
...
$viewtopic_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start") . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : '') . (($highlight_match) ? "" : ''));
...
$pagination = generate_pagination(append_sid("{$phpbb_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id" . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : '') . (($highlight_match) ? "" : '')), $total_posts, $config['posts_per_page'], $start);
...
//Work out the title of this ACE page.
$ace_params = "";
if($aceforum_id){
  $ace_topic_title.="View Forum: ".$topic_data['forum_name'];
  $ace_params.="f:$aceforum_id";
}else if($acetopic_id){
  $ace_params.="t:$ace_topic_id";
  $ace_topic_title.="View Topic: ".$topic_data['topic_title'];
}else{
  $ace_topic_title.="View Entire Board";
}
...
$sql = 'SELECT p.post_id
	...
	if($acetopic_id){
  		$sql .= " WHERE p.topic_id = $acetopic_id";
	}else if($aceforum_id){
	...
	$sql.= ((!$auth->acl_get('m_approve', $forum_id)) ? 'AND p.post_approved = 1' : '') . "
...
//can then ask for only posts highest than that next time.
$ace_highest_post = 0;
while ($row = $db->sql_fetchrow($result))
{
	$post_list[$i] = (int) $row['post_id'];
	if($post_list[$i] > $ace_highest_post){
		$ace_highest_post=$post_list[$i];
	}
	$i++;
}
if($store_reverse){
  $pl2=$post_list;
  $post_list = array();
  //Reverse all those things.
  for($n=0;$n<sizeof($pl2);$n++){
    $post_list[$n]=$pl2[sizeof($pl2)-$n-1];
  }
} 
$db->sql_freeresult($result);
...
$template->assign_vars(array(
	'ACE_HIGHEST_POST' 	=> $ace_highest_post,
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$postrow = array(
		...
		'POST_SUBJECT'		=> $row['post_subject'],
		'POST_EVENTDATE'		=> getEventDate($row['topic_id'],1),
		'MESSAGE'			=> $message,
		...
		'TOPIC_ID'			=> $row['topic_id'],
		'FORUM_ID'			=> $row['forum_id'],
		'TOPIC_TITLE'			=> getTopicTitle($row['topic_id']),
		'FORUM_NAME'			=> getForumName($row['forum_id']),
